...
const IntegrationsTableStatement = `-- Main table storing integration package information. Each integration represents a complete Fleet package.
...
const PolicyTemplatesTableStatement = `-- Policy templates offered by integration packages. Related to integrations via foreign key.
...
const DataStreamsTableStatement = `-- Data streams within integration packages. Related to integrations via foreign key.
...
const StreamsTableStatement = `-- Individual streams within data stream manifests. Related to data_streams via foreign key.
...
const VarsTableStatement = `-- Configuration variables used throughout the package. This is a master table for all variables.
...
const IntegrationVarsTableStatement = `-- Join table linking integrations to their configuration variables.
...
const PolicyTemplateVarsTableStatement = `-- Join table linking policy templates to their configuration variables.
...
const PolicyTemplateInputVarsTableStatement = `-- Join table linking policy template inputs to their configuration variables.
...
const StreamVarsTableStatement = `-- Join table linking streams to their configuration variables.
...
const FieldsTableStatement = `-- Elasticsearch field definitions used in data streams and transforms.
...
const TransformsTableStatement = `-- Elasticsearch transform configurations within integration packages.
...
const PolicyTemplateInputsTableStatement = `-- Input configurations for policy templates. Related to policy_templates via foreign key.
...
const PolicyTemplateCategoriesTableStatement = `-- Categories associated with policy templates. Join table for many-to-many relationship.
...
const IntegrationCategoriesTableStatement = `-- Categories associated with integrations. Join table for many-to-many relationship.
...
const IntegrationIconsTableStatement = `-- Icons associated with integrations. Related to integrations via foreign key.
...
const IntegrationScreenshotsTableStatement = `-- Screenshots associated with integrations. Related to integrations via foreign key.
...
const PolicyTemplateIconsTableStatement = `-- Icons associated with policy templates. Related to policy_templates via foreign key.
...
const PolicyTemplateScreenshotsTableStatement = `-- Screenshots associated with policy templates. Related to policy_templates via foreign key.
...
const VarOptionsTableStatement = `-- Options for select-type variables. Related to vars via foreign key.
...
const DataStreamFieldsTableStatement = `-- Join table linking data streams to their field definitions.
...
const TransformFieldsTableStatement = `-- Join table linking transforms to their field definitions.
...
const TransformDestAliasesTableStatement = `-- Aliases for transform destination indices. Related to transforms via foreign key.
...
const DiscoveryFieldsTableStatement = `-- Fields associated with package discovery capabilities. Related to integrations via foreign key.
...
const BuildManifestsTableStatement = `-- Build configuration for integration packages. Related to integrations via foreign key.
...
const ChangelogsTableStatement = `-- Version history for integration packages. Related to integrations via foreign key.
...
const ReleasesTableStatement = `-- Individual releases within changelogs. Related to changelogs via foreign key.
...
const ChangesTableStatement = `-- Individual changes within releases. Related to releases via foreign key.
...
const IngestPipelinesTableStatement = `-- Ingest pipeline configurations within data streams. Related to data_streams via foreign key.
...
var Creates = [...]string{
	IntegrationsTableStatement,
	PolicyTemplatesTableStatement,
	DataStreamsTableStatement,
	StreamsTableStatement,
	VarsTableStatement,
	IntegrationVarsTableStatement,
	PolicyTemplateVarsTableStatement,
	PolicyTemplateInputVarsTableStatement,
	StreamVarsTableStatement,
	FieldsTableStatement,
	TransformsTableStatement,
	PolicyTemplateInputsTableStatement,
	PolicyTemplateCategoriesTableStatement,
	PolicyTemplateDataStreamsTableStatement,
	IntegrationCategoriesTableStatement,
	IntegrationIconsTableStatement,
	IntegrationScreenshotsTableStatement,
	PolicyTemplateIconsTableStatement,
	PolicyTemplateScreenshotsTableStatement,
	VarOptionsTableStatement,
	DataStreamFieldsTableStatement,
	TransformFieldsTableStatement,
	TransformDestAliasesTableStatement,
	DiscoveryFieldsTableStatement,
	BuildManifestsTableStatement,
	ChangelogsTableStatement,
